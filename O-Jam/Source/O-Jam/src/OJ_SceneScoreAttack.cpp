...
void OJ_SceneScoreAttack::handlePlayerInput(OJ_Player * _player, Joystick * _joystick){
	// Calculate movement
	glm::vec2 movement(0);
...
void OJ_SceneScoreAttack::handleStancing(){
	if(playerOne == nullptr || playerTwo == nullptr){
		return;
	}
	// pull
...
void OJ_SceneScoreAttack::unload() {
	mainShader->unload();
	textShader->unload();
	font->unload();
	uiLayer->unload();

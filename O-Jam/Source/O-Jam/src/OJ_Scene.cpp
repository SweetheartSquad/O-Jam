...
void OJ_Scene::render(vox::MatrixStack* _matrixStack, RenderOptions* _renderOptions) {
	...
	if(test2 != -1){
		glUniform1i(test2, arena->waveNumber % 5);
		checkForGlError(0,__FILE__,__LINE__);
	}
	if(test3 != -1){
		glUniform1f(test3, std::abs(OJ_ResourceManager::songs["funker"]->getAmplitude()*OJ_ResourceManager::songs["funker"]->getAmplitude()*std::min(arena->waveNumber, 3)*0.5f));
		checkForGlError(0,__FILE__,__LINE__);
	}
